...
uintptr_t XUtil::FindPattern(uintptr_t StartAddress, uintptr_t MaxSize, const byte *Bytes, const char *Mask)
{
	auto compare = [](const byte *Data, const byte *Bytes, const char *Mask)
	{
	...
	{
		if (compare((byte *)(StartAddress + i), Bytes, Mask))
...
void XUtil::PatchMemory(uintptr_t Address, byte *Data, size_t Size)
{
	DWORD d = 0;
