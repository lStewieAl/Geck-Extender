...
public:
	...
	struct Data
	{
		...
		float			totalWgCurrent;		// 008	OnEquip: at the begining, stored into unk3 and forced to -1.0 before checking owner
		float			totalWgLast;		// 00C
...
public:
	...
	static ExtraHealth* Create(float health);
};
...
public:
	...
	static ExtraHotkey* Create(UInt8 index);
};
...
public:
	...
	~ExtraRadius();
	float radius;
	static ExtraRadius*Create(float inrad);
};
...
class ExtraCellWaterType : BSExtraData
{
public:
	ExtraCellWaterType();
	~ExtraCellWaterType();
...
public:
	...
	static ExtraLinkedRef* Create(TESObjectREFR* ref);
};
class ExtraCellImageSpace : BSExtraData
{
public:
	ExtraCellImageSpace();
	~ExtraCellImageSpace();
...
class ExtraEncounterZone : BSExtraData
{
public:
	BGSEncounterZone* zone;
};

class ExtraLastFinishedSequence : BSExtraData
{
public:
...
class ExtraDismemberedLimbs : BSExtraData
{
public:
...
public:
	...
	float		posVector[3];	// 0C
	float		rotVector[3];	// 18
};
