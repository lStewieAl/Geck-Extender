...
public:
	...
	virtual TESPackage *GetInterruptPackage();		// 083 - GetInterruptPackage
	...
	virtual TESPackage *GetStablePackage();			// 08B - GetStablePackage
	...
	virtual void	Unk_A1();
	virtual void	Unk_A2();
	virtual bhkCharacterController *GetCharacterController();						// returns the current animation from Unk0138 aparently.
	virtual void	Unk_A4();
	virtual void	Unk_A5();
	virtual void	Unk_A6();
	virtual void	Unk_A7();
	...
	virtual void	SetKnockedState(SInt8 state);
	...
	PackageInfo		currentPackage;		// current package ?
...
struct AnimData
{
	...
	float							unk020;				// 020
	UInt32							unk024;				// 024
	NiNode							*niNode028;			// 028
	NiNode							*niNode02C;			// 02C
	NiNode							*niNode030;			// 030
	NiNode							*niNode034;			// 034
	NiNode							*niNode038;			// 038
	UInt32							unk03C[2];			// 03C
	NiNode							*niNode044;			// 044
...
public:
	...
	PackageInfo				interruptPackage;	// 0E4	I suspect interrupt package
	...
	UInt8					grenadeEquipped;// 124	OneHandGrenade equipped
	UInt8					mineEquipped;	// 125	OneHandMine equipped
	UInt8					thrownEquipped;	// 126	OneHandThrown equipped
	UInt8					byt127;			// 127
	UInt8					byt128;			// 128
	UInt8					byt129;			// 129
	UInt8					byt12A;			// 12A
...
public:
	...
	UInt32								unk428[3];			// 428
	bhkSimpleShapePhantom				*ptr434;			// 434
	UInt32								unk438;				// 438
	float								unk43C;				// 43C
	float								radsSec440;			// 440
	UInt32								unk444[10];			// 444
};

struct ProcessManager
{
	UInt32					unk00;				// 00
	NiTArray<MobileObject*>	objects;			// 04
	UInt32					beginOffsets[4];	// 14	0: High, 1: Mid-High, 2: Mid-Low, 3: Low
	UInt32					endOffsets[4];		// 24
	UInt32					unk34[11];			// 34
	tList<BSTempEffect>		tempEffects;		// 60
	UInt32					unk68[6];			// 68
	tList<Actor>			highActors;			// 80
	//	More?

	static ProcessManager* GetSingleton() { return (ProcessManager*)0x011E0E80; }
};