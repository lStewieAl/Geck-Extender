...
ScriptEventList* Script::CreateEventList(void)
{
	...
	try
	{
		...
		result = (ScriptEventList*)ThisCall(0x005ABF60, this);	// 4th sub above Script::Execute (was 1st above in Oblivion) Execute is the second to last call in Run
#elif RUNTIME_VERSION == RUNTIME_VERSION_1_4_0_525ng
		result = (ScriptEventList*)ThisCall(0x005AC110, this);	// 4th sub above Script::Execute (was 1st above in Oblivion) Execute is the second to last call in Run
#else
#error
#endif
	} catch(...) {}
...
Script::RefVariable* ScriptBuffer::ResolveRef(const char* refName)
{
	...
	{
		TESForm* form = LookupFormByName(refName);
		if (form)
		{
			TESObjectREFR* refr = DYNAMIC_CAST(form, TESForm, TESObjectREFR);
