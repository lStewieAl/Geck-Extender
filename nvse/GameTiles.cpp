...
Tile *Tile::GetChild(const char *childName)
{
	...
	while (node)
	{
		if (node->child && ((tileName == "*") || (!_stricmp(node->child->name.m_data, tileName.c_str()))))
		{
			if (foundCount == childIndex) return node->child;
			foundCount++;
		}
		node = node->next;
	}
...
void Tile::Dump(void)
{
	...
	for(UInt32 i = 0; i < values.size; i++)
	{
		...
		if(val->str)
			_MESSAGE("%d  %s: %s", val->id, traitName, val->str);
		else if(val->action)
			_MESSAGE("%d  %s: action %08X", val->id, traitName, val->action);
		else
			_MESSAGE("%d  %s: %f", val->id, traitName, val->num);
	}
