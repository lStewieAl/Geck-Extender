...
ExtraContainerChanges::Data* ExtraContainerChanges::Data::Create(TESObjectREFR* owner)
{
	...
	if (data) {
		data->owner = owner;
		data->objList = NULL;
		data->totalWgCurrent = 0.0;
		data->totalWgLast = 0.0;
	}
	return data;
}
...
void ExtraContainerChanges::DebugDump()
{
	...
	if (data && data->objList)
	{
		...
		{
			_MESSAGE("Type: [%08X] %s CountDelta: %d [%08X]", entry.Get()->type->refID, GetFullName(entry.Get()->type), entry.Get()->countDelta, entry.Get());
			gLog.Indent();
...
ExtraHealth* ExtraHealth::Create(float health) 
{
	ExtraHealth* xHealth = (ExtraHealth*)BSExtraData::Create(kExtraData_Health, sizeof(ExtraHealth), s_ExtraHealthVtbl);
...
char * GetExtraDataName(UInt8 ExtraDataType) {
	switch (ExtraDataType) {			
		...
		case	kExtraData_WeaponModFlags				: return "WeaponModFlags"; break;
	};
	return "unknown";
}
...
ExtraHotkey* ExtraHotkey::Create(UInt8 index = 0)
{
...
ExtraLinkedRef* ExtraLinkedRef::Create(TESObjectREFR* ref)
{
	ExtraLinkedRef* link = (ExtraLinkedRef*)BSExtraData::Create(kExtraData_LinkedRef, sizeof(ExtraLinkedRef), 0x01015CC0);
	link->linkedRef = ref;
	return link;
}
ExtraRadius* ExtraRadius::Create(float inrad)
{
		ExtraRadius* radius = (ExtraRadius*)BSExtraData::Create(kExtraData_Radius, sizeof(ExtraRadius), 0x01015208);
		radius->radius = inrad;
		return radius;
}
