...
#error
	...
	struct ChildNode
	{
		...
		ChildNode *Advance(UInt32 times)
		{
			ChildNode *result = this;
			while (times--)
			{
				result = result->prev;
	...
	ChildNode		*GetNthChild(UInt32 index);
	...
	void			SetFloat(UInt32 id, float fltVal);
	void			SetString(UInt32 id, const char *strVal);
	void			HandleMenuInput(int tileID);
	Menu			*GetParentMenu();
